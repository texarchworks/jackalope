...
export default function MyWork({ user, projects, goProj, goEditTask, onUpdateTask }) {
  ...
  const ResolveBtn = ({ onClick, dark }) => (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }} title="Close / Resolve"
      style={{ width: 20, height: 20, borderRadius: 4, border: `1px solid ${dark ? "#7f1d1d" : "#252535"}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: dark ? "#FCA5A5" : "#5E5E72", flexShrink: 0, transition: "all .15s" }}
      ...
      onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = dark ? "#FCA5A5" : "#5E5E72"; e.currentTarget.style.borderColor = dark ? "#7f1d1d" : "#252535"; }}>
      ✓
    </button>
  );
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: "#5E5E72" }}>{user.role} · {active.length} active items across {Object.keys(byProj).length} project{Object.keys(byProj).length !== 1 ? "s" : ""}</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
        <Stat label="Active Items" value={active.length} color="#3B82F6" icon="◐" />
        <Stat label="Critical" value={active.filter((t) => t.priority === "critical").length} color="#EF4444" icon="▲" />
        <Stat label="Overdue" value={overdue.length} color="#EA580C" icon="⚠" />
        <Stat label="Projects" value={Object.keys(byProj).length} color="#10B981" icon="▣" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(Object.keys(bySta).length || 1, 5)}, 1fr)`, gap: 12, marginBottom: 24 }}>
        {Object.entries(STA).filter(([k]) => bySta[k]?.length > 0).map(([st, cfg]) => (
          <div key={st} style={{ background: "#0F0F16", borderRadius: 10, border: "1px solid #252535", overflow: "hidden" }}>
            <div style={{ padding: "10px 14px", borderBottom: "1px solid #252535", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 11, color: "#5E5E72", fontFamily: M }}>{bySta[st].length}</span>
            </div>
            <div style={{ padding: 8, display: "flex", flexDirection: "column", gap: 5 }}>
              {bySta[st].slice(0, 8).map((t) => {
                const loc = t._p.locs.find((l) => l.id === t.loc);
                return (
                  <div key={t.id} style={{ background: "#14141D", borderLeft: `3px solid ${PRI[t.priority].color}`, borderRadius: "0 6px 6px 0", padding: "7px 10px", position: "relative" }}
                    onMouseEnter={(e) => e.currentTarget.style.background = "#1A1A28"} onMouseLeave={(e) => e.currentTarget.style.background = "#14141D"}>
                    <div onClick={() => goEditTask(t._p.id, t)} style={{ cursor: "pointer" }}>
                      ...
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        ...
                        {t.dueDate && <span style={{ fontSize: 9, color: t.dueDate < today ? "#F87171" : "#5E5E72", fontFamily: M, marginLeft: "auto" }}>{t.dueDate}</span>}
                      </div>
                    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
        {Object.values(byProj).map(({ p: proj, ts }) => {
          ...
          return (
            <div key={proj.id} onClick={() => goProj(proj.id)} style={{ background: "#0F0F16", borderRadius: 10, border: "1px solid #252535", overflow: "hidden", cursor: "pointer", transition: "border .15s" }}
              onMouseEnter={(e) => e.currentTarget.style.borderColor = proj.color} onMouseLeave={(e) => e.currentTarget.style.borderColor = "#252535"}>
              <div style={{ padding: "14px 18px", borderBottom: "1px solid #252535", display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{proj.name}</div><div style={{ fontSize: 11, color: "#5E5E72" }}>{ts.length} items</div></div>
                {cr > 0 && <div style={{ background: "#FEF2F2", borderRadius: 6, padding: "3px 7px", fontSize: 10, color: "#EF4444", fontWeight: 600 }}>▲ {cr}</div>}
              </div>
              <div style={{ padding: "8px 14px" }}>
                {ts.filter((t) => t.status !== "resolved").slice(0, 4).map((t) => (
                  <div key={t.id} style={{ padding: "5px 8px", borderLeft: `2px solid ${PRI[t.priority].color}`, marginBottom: 3, background: "#14141D", borderRadius: "0 4px 4px 0", fontSize: 11, display: "flex", alignItems: "center", gap: 4 }}>
